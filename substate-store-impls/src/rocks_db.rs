...
impl RocksdbSubstateStore {
    ...
    fn get_column_family_descriptors(root: PathBuf) -> Vec<ColumnFamilyDescriptor> {
        match DB::list_cf(&Options::default(), root.as_path()) {
            Ok(cfs) => {
    ...
    pub fn with_options(options: &Options, root: PathBuf) -> Self {
        ...
        let cfs = Self::get_column_family_descriptors(root.clone());
